...
uint32_t *indtable;
int8_t* globaltable;
int8_t* tournamenttable;
//global branch taken counter
uint8_t  branch_history_register;
...
void PredictorInit() {
    ...
    globaltable = new int8_t[1<< 2];
    tournamenttable= new int8_t[1<<GSHARE_SIZE];
...
void PredictorRunACycle() {
    ...
    for (int i = 0; i < cycle_info->num_fetch; i++) {
        ...
        if (runs == 0 && uop->type & IS_BR_CONDITIONAL) {
            ...
            // misprediction penalty
         	bool globaltaken=globaltable[(branch_history_register&3)]>=0;  
         	if(tournamenttable[gidx]>=0)
         	pred1=gpred;
         else
         	pred1=globaltaken;

         	assert(report_pred(fe_ptr,false,globaltaken));
        ...
        else if(runs ==3 && uop->type & IS_BR_CONDITIONAL){
        	uint32_t gidx = (/*brh_fetch ^*/ uop->pc) & ((1 << GSHARE_SIZE) - 1);
            bool gpred = (gtable[gidx] >= 0);
            bool pred1;
         	bool globaltaken=globaltable[(branch_history_register&3)]>=0;  
         	...
         	assert(report_pred(fe_ptr,false,pred1));
        }
        else if(runs ==4 && uop->type & IS_BR_CONDITIONAL){
        	uint32_t gidx = (brh_fetch ^ uop->pc) & ((1 << GSHARE_SIZE) - 1);
            bool gpred = (gtable[gidx] >= 0);
            bool pred1;
         	bool globaltaken=globaltable[(branch_history_register&3)]>=0;  
    ...
    for (int i = 0; i < cycle_info->num_retire; i++) {
        ...
        if (runs != 1 && uop->type & IS_BR_CONDITIONAL) {
            uint32_t gidx = (/*brh_retire ^*/ uop->pc) & ((1 << GSHARE_SIZE) - 1);
            if(runs==4)
            	gidx=(brh_retire ^ uop->pc) & ((1 << GSHARE_SIZE) - 1);
            bool t = uop->br_taken;
            

            if(t ){
            	branch_history_register=(branch_history_register<<1)+1;
             	if((globaltable[(brh_retire&3)]>=0)&&!(gtable[gidx]>=0) &&tournamenttable[gidx]>-2){
             		tournamenttable[gidx]--;
             	}
             	if(!(globaltable[(brh_retire&3)]>=0)&&(gtable[gidx]>=0) &&tournamenttable[gidx]<1){
             		tournamenttable[gidx]++;
             	}



             	if(globaltable[(brh_retire&3)]<1)
             		globaltable[(brh_retire&3)]++;
             }
            else{
            	branch_history_register=branch_history_register<<1;
            	if((globaltable[(brh_retire&3)]>=0)&&!(gtable[gidx]>=0) &&tournamenttable[gidx]<1){
             		tournamenttable[gidx]++;
             	}
             	if(!(globaltable[(brh_retire&3)]>=0)&&(gtable[gidx]>=0) &&tournamenttable[gidx]>-2){
...
void PredictorRunEnd() {
    runs ++;
    if (runs < 5) // set rewind_marked to indicate that we want more runs
        rewind_marked = true;
}
